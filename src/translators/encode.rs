...
use crate::utils::{slurp_file_certificate, slurp_file_string, slurp_file, write_file, JWTError, JWTResult};
use atty::Stream;
...
fn create_header(alg: Algorithm, kid: Option<&String>, no_typ: bool, x5c: Option<&String>) -> Header {
    ...
    match x5c
    {
        Some(x5c) => {
            ...
            my_vec.push(slurp_file_certificate(x5c));
            header.x5c = Some(my_vec.iter().map(|s| s.to_string()).collect::<Vec<String>>());
        }
        None => ()
    };
...
pub fn encode_token(arguments: &EncodeArgs) -> JWTResult<String> {
    ...
    let header = create_header(algorithm, arguments.kid.as_ref(), arguments.no_typ, arguments.x5c.as_ref());
...
mod tests {
    ...
    fn creates_jwt_header_with_kid() {
        ...
        let x5c = String::from("");
        let result = create_header(algorithm, Some(&kid), false,Some(&x5c) );
    ...
    fn creates_jwt_header_without_kid() {
        ...
        let kid = None;
        let x5c = String::from("");
        let result = create_header(algorithm, kid, false,Some(&x5c));
        let mut expected = Header::new(algorithm);
    ...
    fn creates_jwt_header_without_typ() {
        ...
        let kid = None;
        let x5c = String::from("");
        let result = create_header(algorithm, kid, true,Some(&x5c));
        let mut expected = Header::new(algorithm);
